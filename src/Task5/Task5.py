...
def main():
    ...
    def part3():
        print("\n")
        print("КФ Меллера")
        print("Проверка на многочленах степеней 5, 7, 9: \n")
        fs = [lambda x: 6 * x ** 5 + 5 * x ** 4 - 4 * x ** 3 - 3 * x ** 2 + 2 * x + 1,
              lambda x: 16 * x ** 7 - 21 * x ** 6 + 3 * x ** 5 - 10 * x ** 4 + 16 * x ** 3 - 6 * x ** 2 + 18 * x - 8,
              lambda x: 2000 * x ** 9 + 77 * x ** 6 - 20 * x ** 3 - 0.7]
        ...
        r = meller(fs[0], 3)
        print("{:<20} {:<10} {:<30}".format("Степень многочлена", "N", "Погрешность"))
        print("{:<20} {:<10} {:<30}".format("5", "3", f"{abs(r - 4.319689898685965702886134652009316465771107924141)}"))

        r = meller(fs[1], 4)
        print("{:<20} {:<10} {:<30}".format("Степень многочлена", "N", "Погрешность"))
        print("{:<20} {:<10} {:<30}".format("7", "4", f"{abs(r + 66.95519342963246839473508710614440521945217282418)}"))

        r = meller(fs[2], 5)
        print("{:<20} {:<10} {:<30}".format("Степень многочлена", "N", "Погрешность"))
        print("{:<20} {:<10} {:<30}".format("9", "5", f"{abs(r - 73.3954583694915445335835060418673861320563700929)}"))
        ...
        print(f"Результаты по Меллеру для f(x) = 1 / (1 + x^2): \n")

        for n in ns:
            cur = meller(lambda x: 1 / (1 + x ** 2), n)
            print("{:<20} {:<40} {:<40}".format("N", "Результат", "Погрешность"))
            print("{:<20} {:<40} {:<40}".format(n, cur, abs(cur - 2.2214414690791831235079404950303468493073108446879)))
